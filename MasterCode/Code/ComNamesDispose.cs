...
namespace MasterCode.Code
{
    public class ComNamesDispose
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<string> GetComNames()
        {
            List<string> result = new List<string>();
            string[] names =  GetHarewareInfo(HardwareEnum.Win32_PnPEntity, "Name");

            for(int counter = 0; counter < names.Length; counter++)
            {
                int begin = names[counter].IndexOf('(') + 1;
                int end = names[counter].IndexOf(')') - 1;
                string name = names[counter].Substring(begin, end - begin + 1);
